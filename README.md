...
1. **网络链接**：直接在界面里粘贴图片的网络链接即可。
2. **本地上传**：
    *   **零配置模式（默认）**：直接使用内置的临时图床，无需任何设置，上传即用，图片会传到https://111666.best/ 注意隐私谨慎使用。
    *   **Cloudflare R2**：如果你想将图片永久保存在自己的空间，可以在“设置”里配置 R2 参数。配置后，上传功能将优先使用你的私有存储。
